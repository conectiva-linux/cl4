...
Summary(pt_BR): Inittab e scripts /etc/rc.d
Summary(es): Inittab y scripts /etc/rc.d
Name: initscripts
%define version 4.19
Version: %{version}
Copyright: GPL
...
Source1: lang.csh
Source2: inputrc.cl
Patch: initscripts-4.19-cl.patch
BuildRoot: /var/tmp/initbld
Requires: mingetty, bash, gawk, sed, mktemp, e2fsprogs, console-tools, procps
Requires: modutils >= 2.1.85-3
...
%description -l pt_BR
Este pacote contém as scripts usadas para o boot do sistema, mudar
níveis de execução (run level), e desligar (shutdown) o sistema
corretamente. Também contém as scripts que ativam/desativam a
maioria das interfaces de rede.

%description -l es
Este paquete contiene las scripts usadas para que el arranque
del sistema cambie niveles de ejecución (run level), y para que
desconecte (shutdown) el sistema correctamente. También contiene las
scripts que activan/desactivan la mayoría de las interfaces de red.
...
install -m755 $RPM_SOURCE_DIR/lang.csh $RPM_BUILD_ROOT/etc/profile.d
install -m644 $RPM_SOURCE_DIR/inputrc.cl $RPM_BUILD_ROOT/etc/inputrc
...
- get rid of tty command (init.d/functions)
- add option -r to umount (init.d/halt)

* Thu Jul 01 1999 Wanderlei Antonio Cavassin <cavassin@conectiva.com>
- backported fixes from initscripts-4.23
- prefdm link defaults to kdm

* Mon Jun 28 1999 Wanderlei Antonio Cavassin <cavassin@conectiva.com>
- inputrc with key codes to rxvt

* Sat Jun 26 1999 Wanderlei Antonio Cavassin <cavassin@conectiva.com>
- fixes status function to print correctly more than one pid

* Sun Jun 20 1999 Wanderlei Antonio Cavassin <cavassin@conectiva.com>
- removed attemps to make symlinks in /boot to module-info e system.map
- loadkeys runs quietly in rc.sysinit

* Fri Jun 18 1999 Wanderlei Antonio Cavassin <cavassin@conectiva.com>
- new inputrc.cl

* Tue Jun 08 1999 Wanderlei Antonio Cavassin <cavassin@conectiva.com>
- creation of /var/run/utmp fixed

* Sun Jun 06 1999 Wanderlei Antonio Cavassin <cavassin@conectiva.com>
- fix typo in rc.local patch

* Sun Jun  6 1999 Conectiva <dist@conectiva.com>
- Recompiled with glibc 2.1.x, egcs 1.1.x, rpm 3.0.x and kernel 2.2.x

* Thu Jun 03 1999 Wanderlei Antonio Cavassin <cavassin@conectiva.com>
- included i18n support and a nice inputrc
